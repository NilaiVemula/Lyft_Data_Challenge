...
driver_attributes <- driver_attributes %>% select(-career_length_in_days) %>% select(-average_ride_duration) %>% select(-average_ride_distance) %>% remove_rownames %>% column_to_rownames(var="driver_id")
