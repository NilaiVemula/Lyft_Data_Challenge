...
a <- (max(last_timestamp$timestamp))
a
```

```{r}
sorted_driver <- arrange(driver_ids,driver_onboard_date)
sorted_driver
```

Driver onboard date ranges from 3/28/2016 to 5/15/2016

```{r}
sorted_ride_timestamps <- arrange(ride_timestamps, desc(timestamp))
sorted_ride_timestamps
```

Timestamps range from 3/28/2016 to 6/27/2016.
...
```{r}
latest_time <- filter(full, event == "dropped_off_at")
latest_time
```

```{r}
deltas <- latest_time %>% mutate(time_from_employment_to_current_ride = timestamp-driver_onboard_date)
deltas
```

```{r}
driver_lengths <- select(deltas, driver_id, time_from_employment_to_current_ride, time_of_current_ride=timestamp )
driver_lengths
```
...
time_between_sessions_set <- driver_lengths %>%
  drop_na() %>% 
  group_by(driver_id) %>% 
...
```{r}
quitters <- left_join(time_between_sessions_set,career_length_so_far)

quitters_plot<- quitters %>% 
  ggplot(aes(length)) +
  geom_histogram(binwidth=10080)
quitters_plot
```
...
```{r}
quit_avg <- quitters %>% ungroup() %>% summarise(mean(length))
quit_avg
```
